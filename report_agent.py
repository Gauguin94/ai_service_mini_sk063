...
def _generate_executive_summary(
    ...
    """
    Executive Summary 생성 (리포트 최상단 요약)
    ...
    Returns:
        str: Executive Summary 텍스트
    """
    from datetime import datetime
    current_month = datetime.now().strftime("%Y년 %m월")
    
    sys = f"""Create a concise Executive Summary for this investment report. Current date: {current_month}

⚠️ CRITICAL - USE NATURAL KOREAN:
Write in professional, fluent Korean with proper tense.

Structure (150-200 words):
1. **현재 상황** (2-3 sentences): 기업의 최근 동향 및 주요 이슈
2. **핵심 발견** (3-4 bullet points): 
   - 재무/밸류에이션 하이라이트
   - 사업/제품 주요 변화
   - 시장 포지션
3. **투자 의견** (1-2 sentences): 간단한 방향성 (상세 내용은 투자 추천 섹션 참조)

Use natural tense:
- Past events: '~했다', '~을 기록했다'
- Current status: '~하고 있다', '~으로 평가된다'
- Future outlook: '~할 것으로 예상된다'

Be objective and data-driven. Focus on key insights only."""

    user = json.dumps({
        "company": entity.get("name"),
        "ticker": entity.get("ticker"),
        "business_summary": sections.get("business", "")[:800],
        "value_summary": sections.get("value", "")[:400],
        "tech_summary": sections.get("tech", "")[:400],
        "risk_summary": sections.get("risks", "")[:400],
        "revenue_data": tables.get("rev_compare", []),
        "valuation": tables.get("finviz_blocks", {}).get("valuation", {}),
        "top_sources": [
    ...
    try:
        ...
        return (
            f"**현재 상황**: {name}({ticker})에 대한 {perspective} 관점 분석을 수행했습니다. "
            f"최근 뉴스 및 공시 자료를 바탕으로 사업 동향, 재무 상황, 투자 리스크를 종합 분석했습니다.\n\n"
            f"**핵심 발견**:\n"
            f"- 다양한 소스({len(sources)}개)를 기반으로 분석 수행\n"
            f"- {perspective.upper()} 관점에서 종합 평가\n"
            f"- 주요 리스크 요인 식별\n\n"
            f"**투자 의견**: 상세한 분석 내용은 아래 섹션을 참고하시기 바랍니다."
        )
...
def save_docx(state: Dict[str, Any], out_path: Optional[str] = None) -> str:
    ...
    # Executive Summary (report_md에서 추출)
    report_md = state.get("report_md", "")
    if "## Executive Summary" in report_md:
        exec_start = report_md.find("## Executive Summary")
        exec_end = report_md.find("## TL;DR", exec_start)
        if exec_start != -1 and exec_end != -1:
            exec_text = report_md[exec_start:exec_end].replace("## Executive Summary", "").strip()
            doc.add_heading("Executive Summary", level=1)
            
            # Executive Summary 텍스트 파싱
