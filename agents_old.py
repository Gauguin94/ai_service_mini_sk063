...
2. **Suspicious Model Names**: Flag model names that seem fabricated or unusual
   - Look for: "Stealth", "Ultra", "Performance Plus", unusual suffixes
   - Compare with known {ticker} product lineup

3. **Invalid Promotions**: Check promotion validity
   - Must have specific dates/periods
   - Must not be expired (before 2024)
...
Return JSON:
{{
  "tense_errors": [list of specific examples],
  "fake_models": [list of suspicious model names],
  "invalid_promos": [list of invalid promotions],
  "duplicates": [list of duplicate entries],
  "score": 0 or 1
}}

Score should be 1 only if ALL categories have NO issues.
"""
...
def extract_revenue_from_text(text: str, ticker: str, openai_client) -> dict:
    ...
    sys = (
        ...
        "Output JSON: {'quarter_label': str, 'last_quarter': float, 'prev_quarter': float, 'yoy': float, 'yoy_quarter_label': str}. "
